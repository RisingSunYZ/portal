var flowSystem = {
    ...
    list: {
        ...
        syncData:function(){
            ...
            mini.confirm("同步数据？", "确定？",
                function (action) {
                    if (action == "ok") {
                        $.GET({
                            url:basePath+'/basedata/flow_system/sync',
        ...
        delByIds:function(ids){
            var _this = this;
            $.POST({ url: basePath+'/basedata/flow_system/dels', data: {ids:ids.join(',')},
                success:function (res) {
                    if(res.code == ReturnCode.SUCCESS){
                        _this.grid.reload();
                        showSucTips("删除成功！");
        ...
        openIpt:function(row, editable){
            var _this = this;
            var title = (row ? (undefined != editable&&!editable?"查看":"编辑") : "添加") + "系统";
            var url = basePath + "/basedata/flow_system/input";
            // 【弹窗模式】 弹窗添加修改页面
            mini.open({
                url: url,
                allowResize:true,
                ...
                onload: function () {
                    if(row!=undefined) {
                        ...
                        iframe.contentWindow.flowSystem.input.setData(row);
                        if(undefined != editable&&!editable){
                            iframe.contentWindow.flowSystem.view.init();
                        }
                    }
                },
        ...
        uniqueCode:function (e) {
            ...
            code = mini.get("#sn").getValue();
            if (code == "" || code == null) {
                e.errorText = "编码不能为空";
                e.isValid = false;
            }else {
                $.POST({
                    url: basePath+"/basedata/flow_system/checkCode",
                    data: {id:id,sn:code},
                    async:false,
    ...
    input: {
        ...
        save:function(){
            ...
            var url = basePath + (formData.id==""?"/basedata/flow_system/insert":"/basedata/flow_system/update");
