var FlowInfo = {
    ...
    list: {
        ...
        bindEvents: function () {
            ...
            // 新增表单
            mini.get('jsAddFormInfo').on('click', function () {
            ...
            mini.get('jsEditFormInfo').on('click', function () {
                ...
                this.openFormInfoIpt(rows[0].id, rows[0].name , rows[0].categoryId);
            }, this);
            ...
            mini.get('jsDoCopyForm').on('click', function () {
                ...
                if(copyFormForm.validate()){
                    mask();
                    $.POST({
                        url: basePath + '/flow/form/form_info/copyCustForm',
                        data:{fromModelKey:formCode, newModelName:newFormName, newCategory:category},
                        success:function(dt){
                            unmask();
                            if(dt.code == '100'){
                                showSucTips("拷贝成功！");
            ...
            this.grid.on('select', function (e) {
                if(e.record.categoryId){
                    var findedNodes = this.tree.findNodes (function (node) {
                        if(node.code == e.record.categoryId)return true;
                    });
        ...
        openFormInfoIpt:function (id, name, catId) {
            addBlankTab({title:(name?name:'自定义') + (id?"修改":"添加"), url: basePath+'/flow/form/form_info/detailUI?formInfoId=' + id + '&categoryId=' + catId+'&formType=1'});
        },
        ...
        activeForm:function(_id) {
            ...
            mask();
            $.ajax({
                url: basePath + '/flow/form/form_item/activeForm',
                dataType: "JSON",
                type: "POST",
                data: {"formInfoId": id},
        ...
        stopForm:function(_id) {
            ...
            showConfirm('你确认停用吗？', function (r) {
                if (r) {
                    mask('正在执行...');
                    var url = basePath + '/flow/form/form_info/stopForm';
                    $.ajax({
                        type: 'POST',
                        dataType: 'JSON',
                        url: url,
        ...
        showCopyForm:function (row) {
            ...
            if (row) {
                mini.get('newFormName').set({value:row.title});

                var nodes = this.copyToCategoryTree.tree.findNodes(function(node){
                    if(node.code == row.categoryId) return true;
                });
        ...
        renderState:function(row){
            // CJ(1, "草稿"), DFB(2, "待发布"), YFB(3, "已发布"), TY(4, "停用")
            var value = row.value;
            // return row.value == '待发布' ? '<span style="color: #ffc600;">'+row.value+'</span>' : (row.value == '草稿' ? '<span style="color: red;">'+row.value+'</span>' : '<span style="color: green;">'+row.value+'</span>');
            if (value == 1) {
                return '<span style="color: red">草稿</span>';
            }
            if (value == 2) {
                return '<span style="color: #ffc600">待发布</span>';
            }
            if (value == 3) {
