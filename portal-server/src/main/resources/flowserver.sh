 ...
 start(){
   ...
   if [ $? -eq "0" ]; then
    ...
    if [ ${DEBUG_MODE} -eq "1" ]; then
      nohup java -jar -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=5008 $APP_NAME --spring.profiles.active=$ENVIRONMENT  --spring.config.location=$PROPERTIES > /dev/null 2>&1 &
    else
