var flowCategory = {
    ...
    list: {
        ...
        delByIds: function (ids) {
            ...
            $.POST({
                ...
                success:function(data){
                    if(data.length ==0) {
                        $.POST({
                            url: basePath + '/basedata/flow_category/dels', data: {ids: ids.join(',')},
        ...
        openIpt: function (row, editable,isAddChildren) {
            ...
            var url = basePath + "/basedata/flow_category/input";
            // 【弹窗模式】 弹窗添加修改页面
            mini.open({
                url: url,
                allowResize: true,
                title: title,
                width: 1110, height: 450,
                onload: function () {
                    if (row != undefined) {
                        if (isAddChildren== undefined) {
                            var iframe = this.getIFrameEl();
                            iframe.contentWindow.flowCategory.input.setData(row);
                        }else{
                            var iframe = this.getIFrameEl();
                            iframe.contentWindow.flowCategory.input.setChildrenData(row);
                        }
                        if (undefined != editable && !editable) {
                            iframe.contentWindow.flowCategory.view.init();
                        }
        ...
        uniqueCode: function (e) {
            ...
            if (code == "" || code == null) {
                ...
                $.POST({
                    url: basePath + "/basedata/flow_category/checkCode",
    ...
    input: {
        ...
        save: function () {
            ...
            var url = basePath + (formData.id == "" ? "/basedata/flow_category/insert" : "/basedata/flow_category/update");
