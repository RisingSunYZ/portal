var flowExtendPriv = {
    ...
    list: {
        ...
        delByIds: function (ids) {
            ...
            $.POST({
                url:basePath+"/org/flowExtendPriv/getAll",
                data:{pid:ids.join(',')},
                success:function(data){
                    if(data.length ==0) {
                        $.POST({
                            url: basePath + '/org/flowExtendPriv/dels', data: {ids: ids.join(',')},
        ...
        openIpt: function (row, editable,isAddChildren) {
            ...
            var url = basePath + "/org/flowExtendPriv/input";
            // 【弹窗模式】 弹窗添加修改页面
            mini.open({
                url: url,
                allowResize: true,
                ...
                onload: function () {
                    if (row != undefined) {
                        if (isAddChildren== undefined) {
                            ...
                            iframe.contentWindow.flowExtendPriv.input.setData(row);
                        }else{
                            var iframe = this.getIFrameEl();
                            iframe.contentWindow.flowExtendPriv.input.setChildrenData(row);
                        }
                        if (undefined != editable && !editable) {
                            iframe.contentWindow.flowExtendPriv.view.init();
                        }
        ...
        uniqueCode: function (e) {
            ...
            if (code == "" || code == null) {
                ...
                $.POST({
                    url: basePath + "/org/flowExtendPriv/checkCode",
    ...
    input: {
        ...
        save: function () {
            ...
            if (_this.form.isValid() == false) return;
            mask('正在保存...');
            var formData = _this.form.getData();
            var url = basePath + (formData.id == "" ? "/org/flowExtendPriv/insert" : "/org/flowExtendPriv/update");
