var positionLevel = {
    ...
    list: {
        ...
        delByIds:function(ids){
            var _this = this;
            $.POST({ url: basePath+'/org/position_level/dels', data: {ids:ids.join(',')},
        ...
        changeStatus:function(_id,_validState){
            var _this = this;
            var url = basePath+"/org/position_level/updateStatus";
            $.POST({
                url: url,
                data: {
                    ids:_id.join(","),
        ...
        openIpt:function(row, editable){
            var _this = this;
            var title = (row ? (undefined != editable&&!editable?"查看":"编辑") : "添加") + "岗级";
            var url = basePath + "/org/position_level/input";
            // 【弹窗模式】 弹窗添加修改页面
            mini.open({
                url: url,
                allowResize:true,
                ...
                onload: function () {
                    if(row!=undefined) {
                        ...
                        iframe.contentWindow.positionLevel.input.setData(row);
                        if(undefined != editable&&!editable){
                            iframe.contentWindow.positionLevel.view.init();
                        }
                    }
                },
        ...
        uniqueCode:function (e) {
            ...
            if (code == "" || code == null) {
                ...
                $.POST({
                    url: basePath+"/org/position_level/checkCode",
    ...
    input: {
        ...
        save:function(){
            ...
            var url = basePath + (formData.id==""?"/org/position_level/insert":"/org/position_level/update");
