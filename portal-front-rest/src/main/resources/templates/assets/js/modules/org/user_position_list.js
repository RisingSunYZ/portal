var userPosition = {
    ...
    list: {
        ...
        syncData:function(){
            ...
            mini.confirm("同步数据？", "确定？",
                function (action) {
                    if (action == "ok") {
                        $.GET({
                            url:basePath+'/org/user_position/sync',
        ...
        delByIds:function(ids){
            var _this = this;
            $.POST({ url: basePath+'/org/user_position/dels', data: {ids:ids.join(',')},
                success:function (res) {
                    if(res.code == ReturnCode.SUCCESS){
                        _this.grid.reload();
                        showSucTips("删除成功！");
        ...
        openIpt:function(row, editable){
            ...
            var url = basePath + "/org/user_position/input";
            // 【弹窗模式】 弹窗添加修改页面
            mini.open({
                url: url,
                allowResize:true,
                ...
                onload: function () {
                    if(row!=undefined) {
                        ...
                        iframe.contentWindow.userPosition.input.setData(row);
                        if(undefined != editable&&!editable){
                            iframe.contentWindow.userPosition.view.init();
                        }
                    }
                },
        ...
        uniqueCode:function (e) {
            ...
            if (code == "" || code == null) {
                ...
                $.POST({
                    url: basePath+"/org/user_position/checkCode",
    ...
    input: {
        ...
        save:function(){
            ...
            var url = basePath + (formData.id==""?"/org/user_position/insert":"/org/user_position/update");
